...
2. Expected:
   - Explanatory text about smell count ranking
...
6. Verify the report contains:
   ...
   - **Files Ranked by Smell Count** table with columns: #, File, Smells, Failure Costs, Appraisal Costs, Total Costs.
...
2. Expected: the "Files Ranked by Smell Count" table shows that file with higher Total Costs than files with fewer smells. The Failure/Appraisal columns break down the costs correctly.
...
**What it does:** Generates a structured JSON report of all scan findings, including smell metadata, PAF cost impact, and per-file analysis. Designed for programmatic analysis and cross-project comparison.

### Test case — JSON export button disabled before scan

1. Open the UXSniffer tool window.
2. Expected: the **Export JSON** button is greyed out (disabled).
3. Run a scan that finds smells.
4. Expected: the **Export JSON** button becomes enabled.

### Test case — JSON export generates valid JSON

1. Run a scan with findings.
2. Click **Export JSON**.
3. Expected: a save file dialog appears with a default filename `UXSniffer_<projectName>.json`.
4. Choose a location and save.
5. Open the file in a text editor or JSON viewer.
6. Expected: valid JSON with the following top-level keys:
   - `"project"` — the project name (string)
   - `"basePath"` — the project base path (string)
   - `"timestamp"` — ISO 8601 date-time (string)
   - `"totalFindings"` — number of findings (integer)
   - `"summary"` — object with `"uniqueFilesAffected"` and `"smellCounts"`
   - `"findings"` — array of finding objects
   - `"fileAnalysis"` — array of per-file analysis objects

### Test case — finding entries contain full metadata

1. Export JSON from a scan that detected a "Large file" smell (S25).
2. Open the JSON and inspect the `"findings"` array.
3. Expected: each finding object contains:
   - `"smellName"` — e.g., "Large file"
   - `"fileName"` — the file name
   - `"filePath"` — relative path from project root
   - `"message"` — the inspection message
   - `"smellId"` — e.g., "S25"
   - `"severity"` — e.g., "High"
   - `"refactoring"` — suggested fix text
   - `"costImpact"` — array of cost objects, each with `"costId"`, `"costName"`, `"pafCategory"`, `"relationshipType"`, and `"priority"`

### Test case — file analysis section

1. Export JSON from a scan where one file has multiple smells.
2. Inspect the `"fileAnalysis"` array.
3. Expected: each entry has `"file"`, `"smellCount"`, `"smells"` (array of smell names), and `"totalCostMappings"`.
4. Entries are sorted by `"smellCount"` in descending order.
5. The file with the most smells appears first.

### Test case — summary smell counts

1. Export JSON from a scan with 3 "Large file" findings and 2 "Too many props" findings.
2. Inspect `"summary"` → `"smellCounts"`.
3. Expected: `{"Large file": 3, "Too many props": 2}`.

### Test case — JSON export disabled when no findings

1. Open an empty project or one with only clean files.
2. Run scan — expect "No UX smells found."
3. Expected: the **Export JSON** button remains disabled.

### Test case — cross-project merging workflow

1. Scan Project A and export JSON to `projectA.json`.
2. Scan Project B and export JSON to `projectB.json`.
3. Both files should be valid standalone JSON documents.
4. The `"project"` field distinguishes which findings belong to which project.
5. The `"findings"` arrays can be concatenated to create a combined dataset for cross-project analysis.
